...
static js_value_t *
bare_gtk_widget_sensitive(js_env_t *env, js_callback_info_t *info) {
  int err;
  ...
  if (argc == 1) {
    err = js_get_boolean(env, gtk_widget_get_sensitive(widget), &result);
    assert(err == 0);
  } else {
    bool sensitive;
    err = bare_gtk__read_bool(env, argv[1], "sensitive", &sensitive);
    if (err < 0) return NULL;
